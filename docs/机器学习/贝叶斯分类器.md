...
P(c)=\frac{D_{c}}{D}
$$
c为样本类别。$P(c)$为该类别数据$(D_{c})$在数据集所有类别数据$(D)$中占的比例，也是其出现的概率
## 2. 后验概率
+ 离散式
$$
P(x_{i}|c)=\frac{|D_{c,x_{i}}|}{D_{c}}
$$
意思是c类别下，该特征是$x_{i}$的数据在所有c类别数据中的占比
+ 连续式
$$
...
+ 样本不足，有的样本出现零
拉普拉斯修正
分子加(1), 分母加(构成分母种类的数量N, 对于数据集来说就是类别，对于特征来说就是可取值数量)
## 5. 算法
1. 数据预处理
2. 估计先验概率$P(c)$
3. 估计每个类别条件下各特征下每种取值出现的概率$P(x_{i}|c)$
4. 为每个类别计算概率$P(c|x)$, 选择概率最大的c作为输出
## 最小化风险的角度看贝叶斯分类
